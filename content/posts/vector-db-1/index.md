...
* __Pros__: _Amazing_ [documentation](https://weaviate.io/developers/weaviate) (one of the best out there, including technical details and ongoing experiments). Developer experience is really good, it's very easy to get up and running via Docker, and it provides very fast, sub-millisecond search results, while offering both keyword and vector search functionality.
* __Cons__: Because it's built in Golang, scalability is achieved through Kubernetes and really large datasets may require a fair amount of resources and compute. The cost implications for their fully-managed offering over the long term are unknown due to this factor -- it _could_ cost more to operate in production than Rust-based alternatives like Qdrant and LanceDB (time will tell).
* __My take__: Weaviate has a great user community, and the development team are actively experimenting with extreme scalability (hundreds of billions to a trillion vectors in the index), so it does seem that their target market is large-scale enterprises that have mountains of data on which to do vector search. It's hybrid search features are quite popular, and they are actively publishing [use cases that support data science and machine learning](https://www.youtube.com/watch?v=IRWHa57T-zk) via vector databases, not just search.

### Qdrant

* __Pros__: Although newer than Weaviate, Qdrant also has very good documentation that helps users get up and running via Docker with ease. Built entirely in Rust, it offers APIs that developers can tap into via its Rust, Python and Golang clients, which are the most popular languages for backend devs these days. Due to the underlying power of Rust, its resource utilization seems lower than alternatives built in Golang (at least in my experience). Scalability is currently achieved through partitioning and the Raft consensus protocol, which are standard practices in the database space.
...
* __My take__: I think Qdrant stands poised to become the go-to, first-choice vector search backend for a lot of companies that want to minimize infrastructure costs and leverage the power of a modern programming language, Rust. It's clear that Qdrant's user community is rapidly growing (interestingly, faster than Weaviate's), as per [its GitHub star history!](https://star-history.com/#weaviate/weaviate&qdrant/qdrant&Date). Perhaps developers are all hyped-up about 🦀 Rust?
...
* __My take__: The vector DB market is rapidly evolving, and Chroma seems to be inclined[^7] to adopt a "wait and watch" philosophy, in terms of favouring a serverless/embedded model vs. a full-fledged, cloud-native SaaS solution based on a client-server architecture. We've never yet seen an embedded database successfully be commercialized in the vector database space, so its evolution (alongside LanceDB, described below) will be interesting to watch in terms of which hosting solution becomes the most popular!!
...
* __Cons__: This is a **very** young database, so a lot of features are under active development, and prioritization of features will be a challenge in the coming year or so due to a small engineering team.
...
* __Pros__: Provides very good hybrid search (BM25 + HNSW), combining the power of keyword search and vector search. Although other vendors like Weaviate also provide keyword + vector search, Vespa was among the first to market with this offering, resulting in a mature and robust search solution.
* __Cons__: Built entirely in Java. For leaner teams that want to modernize their tech stack, databases built in Go or Rust are normally a better bet for the longer term, in terms of resource utilization, memory consumption and scalability.
* __My take__: I believe the world is slowly but surely moving away from Java-based databases. Almost no new databases these days are being written in Java (Rust is becoming more and more popular for databases due to its performance, memory safety and support for concurrency). As a result, the head start that Vespa once enjoyed may be well be over, with many customers looking ahead to minimize long term infrastructure cost and scale to a trillion vectors.
